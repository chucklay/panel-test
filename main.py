...
selector = pmui.MultiSelect(
    label="Select stocks", sizing_mode="stretch_width", options=stocks.columns.to_list()
)
...
def compute_random_allocations(log_return, num_ports=15000):
    ...
    data["Return"] = expected_return = np.sum(
        (mean_return * normed_weights) * 252, axis=1
    )
    return_covariance = np.cov(log_return[1:], rowvar=False) * 252
    if not return_covariance.shape:
        return_covariance = np.array([[252.0]])
    data["Volatility"] = volatility = np.sqrt(
        (
            normed_weights * np.tensordot(return_covariance, normed_weights.T, axes=1).T
        ).sum(axis=1)
    )
...
def get_volatility(log_ret, weights):
    return np.sqrt(
        np.dot(weights.T, np.dot(np.cov(log_ret[1:], rowvar=False) * 252, weights))
    )
...
def compute_frontier(df, n=30):
    ...
    init_guess = [1.0 / cols for i in range(cols)]
    for possible_return in frontier_ret:
        cons = (
            {"type": "eq", "fun": check_sum},
            {
                "type": "eq",
                "fun": lambda w: (
                    get_return(df.attrs["mean_return"], w) - possible_return
                ),
            },
        )
        result = minimize(
            lambda w: get_volatility(df.attrs["log_return"], w),
            init_guess,
            bounds=bounds,
            constraints=cons,
        )
...
def find_best_allocation(log_return, vol, ret):
    ...
    opt = minimize(
        minimize_difference,
        init_guess,
        args=(vol, ret, log_return, mean_return),
        bounds=bounds,
        constraints=cons,
    )
    ...
    return pd.Series(
        list(opt.x) + [ret, vol],
        index=list(log_return.columns) + ["Return", "Volatility"],
        name="Weight",
    )
...
investment = pmui.IntInput(
    label="Investment Value in $", value=5000, step=1000, start=1000, end=100000
)
year = pmui.DateRangeSlider(
    label="Year",
...
if pn.state.served:
    pmui.Page(main=[main], sidebar=[sidebar], title="Portfolio Optimizer").servable()
if __name__ == "__main__":
    pn.serve(main, show=True, autoreload=True, port=5006)
